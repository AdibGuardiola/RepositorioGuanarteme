...
import { Calendar, Image as ImageIcon, Newspaper, Info, Moon, Sun, Languages } from 'lucide-react';
...
export default function Home() {
    ...
    const navClass = theme === 'dark'
        ...
        : 'backdrop-blur-xl bg-white/80 border-b border-slate-300/50';

    return (
        <div ref={heroRef} className={`min-h-screen ${bgClass} transition-colors duration-500`}>
            {/* Navigation Tabs */}
            <nav className={`sticky top-0 z-50 ${navClass} transition-colors duration-500`}>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className={`p-3 rounded-xl transition-all duration-300 ${theme === 'dark'
                                    ? 'bg-slate-800/50 text-amber-400 hover:bg-slate-700/70'
                                    : 'bg-slate-200/50 text-amber-600 hover:bg-slate-300/70'
                                    }`}
                            ...
                            <button
                                ...
                                className={`flex items-center gap-2 px-4 py-3 rounded-xl font-semibold transition-all duration-300 ${theme === 'dark'
            ...
            {/* Footer */}
            <footer className={`py-12 px-4 border-t transition-colors duration-500 ${theme === 'dark'
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {categorias.map((cat, index) => (
                        <div
                            ...
                            className={`group relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm border border-slate-700/50 transition-all duration-300 p-8 ${cat.count > 0 ? 'cursor-pointer hover:border-amber-500/50 scale-100 hover:scale-[1.02]' : 'opacity-60 grayscale'}`}
                        >
                            ...
                            <div className="relative z-10">
                                ...
                                <p className="text-slate-400 text-sm">
                                    {cat.count === 0 ? 'Próximamente' : `${cat.count} fotos`}
                                </p>
                            </div>
                        </div>
                    ))}
